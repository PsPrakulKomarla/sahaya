...
Builds a professional, fact-based grievance draft from structured inputs. The
composer never invents facts or exaggerates: it only formats verified facts,
...
class GrievanceComposer:
    ...
    def _build_subject(self, category_label: str, application_reference: str | None, language: str) -> str:
        ...
        # Subject is intentionally generic; localization is a future concern.
...
def make_fact(statement: str, source: str | None = None, *, fact_type: FactType = FactType.VERIFIED_FACT) -> GrievanceFact:
...
def application_submitted_fact(reference: str | None, when: datetime | date | None) -> GrievanceFact:
...
def _language_placeholder() -> dict[str, str]:
    # Reserved namespace for future localized labels without inventing facts.
    return {"en": "English", "kn": "Kannada", "hi": "Hindi"}