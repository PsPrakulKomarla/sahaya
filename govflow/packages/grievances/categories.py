...
without modifying ``GrievanceService`` or the API layer. Detection keywords are
provided in English, Kannada, and Hindi so the category classifier is useful
...
class GrievanceCategoryDefinition:
    ...
    label: str
    description: str
    keywords: tuple[str, ...] = field(default_factory=tuple)  # English
    keywords_kn: tuple[str, ...] = field(default_factory=tuple)  # Kannada
    keywords_hi: tuple[str, ...] = field(default_factory=tuple)  # Hindi
...
CATEGORY_DEFINITIONS: dict[GrievanceCategory, GrievanceCategoryDefinition] = {
    ...
    GrievanceCategory.INCORRECT_INFORMATION: GrievanceCategoryDefinition(
        ...
        keywords_kn=("ತಪ್ಪು", "ಅಸಮಂಜಸ"),  # incrementally improved below
...
class GrievanceCategoryRegistry:
    ...
    def detect(self, text: str, language: str = "en") -> GrievanceCategory:
        ...
        if best_score == 0 and language in {"kn", "hi"}:
            # Fall back to canonical (English) keywords for unsupported language.
            for definition in self._definitions.values():
